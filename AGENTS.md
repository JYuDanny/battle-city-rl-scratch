...
### 代码提交前强制检查清单 (AI 代理必须遵守)

每次代码修改完成后，**必须按顺序**执行以下步骤，全部通过后方可提交：

```
1. 运行测试 (pytest)
   ↓ 全部通过
2. 严格安全扫描
   - 搜索所有文件中的 API 密钥、Token、密码、私钥
   - 检查是否泄露个人信息 (邮箱、IP、敏感路径)
   ↓ 无泄漏
3. git commit + push
   ↓
4. 确认 master 分支不动 (当前在 mac 分支开发)
```

**AI 代理每次修改代码后必须主动执行此流程，不得跳过任何一步。**
